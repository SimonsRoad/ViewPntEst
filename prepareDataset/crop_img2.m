function crop_I = crop_img2(orig_img, torso_pt, img_side, scales)
...
%     padd_img = padarray(padd_img, [padd_y_1 0], 'pre');
    ...
    w = round(max(size(padd_img))/2 - size(padd_img,2)/2);
    padd_img = padarray(padd_img, [0 w], 'pre');
    new_img = padarray(padd_img, [0 w], 'post');
    %     new_img = imresize(padd_img, [w, w]);
